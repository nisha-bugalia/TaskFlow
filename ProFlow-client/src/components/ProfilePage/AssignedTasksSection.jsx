...
const AssignedTasksSection = ({ user }) => {
  ...
  useEffect(() => {
    const fetchAllTasks = async () => {
      try {
        const res = await axios.get("http://localhost:5000/task/get-all-tasks", {withCredentials:true}); // or your actual route
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {task.endDate ? new Date(task.endDate).toLocaleString('en-US', { month: 'short', day: 'numeric' }) : ""}
            </span>
          </div>
        </div>
      ))}
